...
def FindingOpponentScreen():
    ...
    while running:
        ...
        if width/2-383 <= mouse[0] <= width/2-33 and height/2+250 <= mouse[1] <= height/2+290: 
            pygame.draw.rect(screen,color_light,[width/2-383,height/2+250,350,40])
        else:
            pygame.draw.rect(screen,color_dark,[width/2-383,height/2+250,350,40]) 
        
        if not t.is_alive():    #if the thread is dead
            t.start()
            put_text('Finding Opponent ... ' , font, WHITE, screen, width/2-120,height/2-50)
        else:   #alive
            put_text('Finding Opponent ... ' + loadingC, font, WHITE, screen, width/2-120,height/2-50)
            
    
        put_text("Return to main menu", font, WHITE, screen, width/2-350, height/2+255)
